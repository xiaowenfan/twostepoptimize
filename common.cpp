...
static inline bool CamProjectionWithDistortion(const double* camera, const double* point, const double* inparameter, double* predictions) {
    // Rodrigues' formula
    double p[3];
...
///更新世界坐标
void BALProblem::UpdatePoint() {
    
    /////////////////////拷贝一个constpoint数组////////世界坐标///
    memcpy(constpoint_, parameters_ + camera_block_size() * num_cameras_, point_block_size() * num_points_ * sizeof(double));
}
...
void BALProblem::ReprojectError(BALProblem& bal_problem) {
    ...
    vector<double> obserrorx(bal_problem.num_observations()), obserrory(bal_problem.num_observations());
    //////////平均误差
