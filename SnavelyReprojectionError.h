...
//         T yd = l1 * p[1] * r2 + p2 * p[1] * p[1] + p1 * p[0] * p[1] + l3 * r2;
        ...
        predictions[0] = focalx * p[0] + cx;//不加入畸变，所以直接用p[0],p[1]
        predictions[1] = focaly * p[1] + cy;
    ...
    static ceres::CostFunction *Create(const double observed_x, const double observed_y,const double conpoint_x,const double conpoint_y,const double conpoint_z) {
        return (new ceres::AutoDiffCostFunction<SnavelyReprojectionError, 2, 6, 3, 9>(//9个内参，3表示一个世界点有xyz三个值，6表示一个相机有3个旋转向量+3个平移向量，第一个6表示有6个residuals
